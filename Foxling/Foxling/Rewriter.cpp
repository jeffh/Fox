...
int Foxling::Rewriter::getRangeSize(const CharSourceRange &Range,
                                    Foxling::Rewriter::RewriteOptions opts) const {
...
int Foxling::Rewriter::getRangeSize(SourceRange Range, Foxling::Rewriter::RewriteOptions opts) const {
  return getRangeSize(CharSourceRange::getTokenRange(Range), opts);
}

/// Fox Addition
std::string Foxling::Rewriter::getRewrittenText(SourceRange Range, int StartDelta, int EndDelta) const {
    FileID StartFileID, EndFileID;
    unsigned StartOff, EndOff;
    StartOff = getLocationOffsetAndFileID(Range.getBegin(), StartFileID);
    EndOff   = getLocationOffsetAndFileID(Range.getEnd(), EndFileID);
...
///
std::string Foxling::Rewriter::getRewrittenText(SourceRange Range) const {
  if (!isRewritable(Range.getBegin()) ||
...
unsigned Foxling::Rewriter::getLocationOffsetAndFileID(SourceLocation Loc,
                                              FileID &FID) const {
...
///
RewriteBuffer &Foxling::Rewriter::getEditBuffer(FileID FID) {
...
/// original buffer.
bool Foxling::Rewriter::InsertText(SourceLocation Loc, StringRef Str,
...
bool Foxling::Rewriter::InsertTextAfterToken(SourceLocation Loc, StringRef Str) {
  if (!isRewritable(Loc)) return true;
  FileID FID;
  unsigned StartOffs = getLocationOffsetAndFileID(Loc, FID);
...
bool Foxling::Rewriter::RemoveText(SourceLocation Start, unsigned Length,
                                   RewriteOptions opts) {
...
/// buffer with a new string.  This is effectively a combined "remove/insert"
/// operation.
bool Foxling::Rewriter::ReplaceText(SourceLocation Start, unsigned OrigLength,
                                    StringRef NewStr) {
  if (!isRewritable(Start)) return true;
  FileID StartFileID;
...
bool Foxling::Rewriter::ReplaceText(SourceRange range, SourceRange replacementRange) {
...
bool Foxling::Rewriter::IncreaseIndentation(CharSourceRange range,
